...
const profile = JSON.parse(
    localStorage.getItem("yellowPawsProfile")
);
...
if (nicknameElement) {
    nicknameElement.textContent = profile.nickname || "Friend";
}

// STREAKS & DAILY REWARDS
const today = new Date().toDateString();
const lastLogin = localStorage.getItem("lastLoginDate");
let streak = Number(localStorage.getItem("learningStreak")) || 0;
...
if (challengeElement) {
    ...
    if ((profile.stars ?? 0) < 25) {
        challengeElement.textContent =
            "🎯 Daily Challenge: Earn 25 Stars";
    }
    else if ((profile.stars ?? 0) < 50) {
        challengeElement.textContent =
            "🎯 Daily Challenge: Reach 50 Stars";
    }
    else if ((profile.stars ?? 0) < 100) {
        challengeElement.textContent =
            "🎯 Daily Challenge: Reach 100 Stars";
    }
    else {
        challengeElement.textContent =
